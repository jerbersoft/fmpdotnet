...
[enumerated and cross-checked](https://github.com/jerbersoft/fmpdotnet/blob/master/docs/superpowers/specs/2026-08-27-endpoint-inventory.md) against two independent
...
| FMP endpoint | Method |
|---|---|
| `stable/biggest-gainers` | `GetBiggestGainersAsync` |
| `stable/biggest-losers` | `GetBiggestLosersAsync` |
| `stable/historical-industry-pe` | `GetHistoricalIndustryPeAsync` |
| `stable/historical-industry-performance` | `GetHistoricalIndustryPerformanceAsync` |
| `stable/historical-sector-pe` | `GetHistoricalSectorPeAsync` |
| `stable/historical-sector-performance` | `GetHistoricalSectorPerformanceAsync` |
| `stable/industry-pe-snapshot` | `GetIndustryPeSnapshotAsync` |
| `stable/industry-performance-snapshot` | `GetIndustryPerformanceSnapshotAsync` |
| `stable/most-actives` | `GetMostActivesAsync` |
...
[endpoint inventory](https://github.com/jerbersoft/fmpdotnet/blob/master/docs/superpowers/specs/2026-08-27-endpoint-inventory.md) splits the remainder section by
...
| Retry | `MaxAttempts` (3) | `BulkMaxAttempts` (**1 — off**) |
...
- **Do not put a generic resilience handler in front of these clients.** `AddStandardResilienceHandler` retries
  429s and trips a circuit breaker that then fast-fails every later request, turning a handful of 429s into
  thousands of skipped symbols. A consumer measured this and had to strip it off both clients. The SDK's own retry
  is a plain `DelegatingHandler`, so `RemoveAllResilienceHandlers()` leaves it intact.
