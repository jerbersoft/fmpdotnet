    ...
    private static object Argument(ParameterInfo parameter)
    {
        ...
        if (type == typeof(int))
        {
            return parameter.Name switch
            {
                ...
                "quarter" => 3,
                // 10, because TechnicalIndicatorsEndpoints.GetAsync validates it, matching the period every
                // measurement in that feature's corpus used and LiveApi.IndicatorPeriodLength on the smoke
                // side. The `_ => 0` default below would be rejected by the facade's own guard and the method
                // would silently vanish from the coverage table — the same reason the `quarter` arm above
                // exists.
                "periodLength" => 10,
                _ => 0, // page, part
            };
