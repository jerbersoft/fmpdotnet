...
**Deferred crefs in this file**, promoted in Task 3 and Task 4 — **four occurrences, three distinct
forms**, because `<c>FmpArticle</c>` appears twice (record summary and `Text`'s `<para>`): `<c>FmpArticle</c>`,
...
`FmpArticle` now exists, so **four** `<c>` placeholders in `src/FmpDotNet/Models/NewsArticle.cs` become
real crefs. Four occurrences, three distinct forms — `<c>FmpArticle</c>` appears **twice**, once in the
record summary and once inside `Text`'s `<para>`. Make exactly these four edits and no others:
...
| `Text` (first `<para>`) | `separates this record from <c>FmpArticle</c> in kind` | `separates this record from <see cref="FmpArticle"/> in kind` |
| `Text` (same `<para>`) | `<c>FmpArticle.Content</c> is markup on 200 of 200 rows` | `<see cref="FmpArticle.Content"/> is markup on 200 of 200 rows` |
...
Run: `dotnet test tests/FmpDotNet.Tests --filter FullyQualifiedName~NewsTests`
Expected: PASS. 19 test methods, 31 cases (17 Facts plus 14 InlineData: the ticker theory 11, the uppercase theory 3).
...
Expected: PASS, 16 cases (11 Facts plus 5 InlineData on the one Theory — 12 is the method count, not the case count).
